//
//  HomeGetUpcomingUseCaseTest.swift
//  MoviesAppTests
//
...
final class HomeGetUpcomingUseCaseTest: XCTestCase {
  private var repository: MockHomeMovieRepository!
  private var sut: HomeGetUpcomingUseCase!
  private var cancellables: Set<AnyCancellable> = []

    override func setUp() {
      super.setUp()
      repository = MockHomeMovieRepository()
      sut = HomeGetUpcomingUseCase(repository: repository)
    }
    ...
    func testGetTopRatedSuccess() throws {
      // Given
      let didGetUpcomingMoviesList: XCTestExpectation = XCTestExpectation()
      ...
      repository.calls.getUpcomingRequest = false
      ...
      sut.execute()
        .sink(
          receiveCompletion: { completion in
            switch completion {
            case .finished:
              didGetUpcomingMoviesList.fulfill()
      ...
      // Then
      wait(for: [didGetUpcomingMoviesList], timeout: 2.0)
      let safeMovieList: [Movie] = try XCTUnwrap(unsafeMovieList)
      XCTAssertTrue(repository.calls.getUpcomingRequest)
